...
namespace FerrumMalleator.Nuntium.Sagas.Handlers
{
    ...
    {
        private readonly Dictionary<Type, SagaHandlerDescriptor> _descriptors = new Dictionary<Type, SagaHandlerDescriptor>();
            ...
            _descriptors[typeof(TMessage)] = new SagaHandlerDescriptor
            {
                ...
                Invoker = async (message, state, provider, ct) =>
                {
                    var handler = provider
                        .GetRequiredService<ISagaHandler<TMessage, TState>>();
                ...
                {
                    Guid.TryParse(correlationId, out Guid stateId);
